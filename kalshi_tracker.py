...
def get_events(series):
    return requests.get(
        ...
        params={"series_ticker":series,"status":"open"},timeout=10
    ).json().get("events",[])

def get_event_details(event):
    return requests.get(f"{BASE}/trade-api/v2/events/{event}",timeout=10).json()["event"]

def get_markets(event):
    return requests.get(
        f"{BASE}/trade-api/v2/markets",
        params={"event_ticker":event,"limit":100},timeout=10
    ).json().get("markets",[])

def get_market_details(ticker):
    return requests.get(f"{BASE}/trade-api/v2/markets/{ticker}",timeout=10).json()["market"]

def get_orderbook(ticker):
    return requests.get(f"{BASE}/trade-api/v2/markets/{ticker}/orderbook",timeout=10).json().get("orderbook",{})

def best(levels):
    if not isinstance(levels,list): return 0
    p=[x[0] for x in levels if isinstance(x,list)]
        ...
        try:
            ...
            for series in SERIES:
                ...
                for e in events:
                    ...
                    # get all market opens to compute event open
                    opens=[]
                    ...
                    for m in markets:
                        ...
                        market_details_cache[m["ticker"]]=md
                        if md.get("open_time"):
